...
class NextJSAPIIntegration {
    ...
    public function register_meta_fields() {
        register_meta('post', 'media_files', array(
            'show_in_rest' => array(
                'schema' => array(
                    'type' => 'array',
                    'items' => array(
            ...
            'single' => true,
            'type' => 'array',
            'description' => 'Media files attached to post',
            'auth_callback' => function() {
                return current_user_can('edit_posts');
            }
        ));
        
        register_meta('post', 'post_media_count', array(
            'show_in_rest' => true,
            'single' => true,
            'type' => 'integer',
    ...
    public function validate_post_creation($prepared_post, $request) {
        if (empty($prepared_post->post_title)) {
            return new WP_Error('empty_title', 'Post title cannot be empty', array('status' => 400));
        }
